...
async function developerRoutes(fastify, options) {
        ...
        if (grant_type === 'refresh_token') {
            ...
            try {
                ...
                if (error)
                    throw error;
                if (!data.session)
                    return reply.code(401).send({ error: 'Invalid credentials or user does not exist' });
                reply.code(200).send({
                    accessToken: data.session.access_token,
    ...
    fastify.get('/developers/me/projects', async (request, reply) => {
    ...
    fastify.post('/developers/projects/:id/usage', async (request, reply) => {
    ...
    fastify.get('/developers/stats', async (request, reply) => {
        const userId = request.user.id;
        try {
