...
// OpenRouter Admin interfaces
interface OpenRouterUserParams {
  userId: string;
}

interface AllocateCreditsBody {
  credits: number;
  source: string;
  reference?: string;
}
...
interface BulkOperationBody {
  userIds: string[];
  credits?: number; // For bulk allocate
  source?: string; // For bulk allocate
  reference?: string; // For bulk allocate
}


export default async function adminRoutes(fastify: FastifyInstance, options: { supabase: SupabaseClient, openRouterProvisioningKey: string }) {
  ...
  fastify.get('/openrouter/users', async (request: FastifyRequest, reply: FastifyReply) => {
    const userId = request.user.id;
    try {
      const { data, error } = await supabase
  ...
  fastify.post(
    '/openrouter/users/:userId/allocate',
    async (request: FastifyRequest<{ Params: OpenRouterUserParams, Body: AllocateCreditsBody }>, reply: FastifyReply) => {
      ...
      try {
        ...
        const result = await openRouterManager.allocateCreditsToUser(
          userId,
          credits,
          source || 'admin_allocation',
          reference || `admin_${adminUserId}_${Date.now()}`,
          supabase,
          fastify.log
        );
  ...
  fastify.put(
    '/openrouter/users/:userId/limit',
    async (request: FastifyRequest<{ Params: OpenRouterUserParams, Body: UpdateLimitBody }>, reply: FastifyReply) => {
      const adminUserId = request.user.id;
      const { userId } = request.params;
  ...
  fastify.post(
    '/openrouter/users/:userId/sync',
    async (request: FastifyRequest<{ Params: OpenRouterUserParams }>, reply: FastifyReply) => {
      const adminUserId = request.user.id;
      const { userId } = request.params;
  ...
  fastify.put(
    '/openrouter/users/:userId/disable',
    async (request: FastifyRequest<{ Params: OpenRouterUserParams }>, reply: FastifyReply) => {
      const adminUserId = request.user.id;
      const { userId } = request.params;
  ...
  fastify.put(
    '/openrouter/users/:userId/enable',
    async (request: FastifyRequest<{ Params: OpenRouterUserParams }>, reply: FastifyReply) => {
      const adminUserId = request.user.id;
      const { userId } = request.params;
      ...
      try {
        ...
        if (error) throw error;
        if (!data) return reply.code(404).send({ error: 'User OpenRouter key not found' });
  ...
  fastify.post(
    '/openrouter/bulk/allocate',
    async (request: FastifyRequest<{ Body: BulkOperationBody }>, reply: FastifyReply) => {
      const adminUserId = request.user.id;
        ...
        for (const userId of userIds) {
          try {
            const result = await openRouterManager.allocateCreditsToUser(
              userId,
              credits,
              source || 'bulk_admin_allocation',
              reference || `bulk_admin_${adminUserId}_${Date.now()}`,
              supabase,
              fastify.log
            );
