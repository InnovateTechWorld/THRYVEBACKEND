...
interface ApiAnalyticsParams {
  apiId: string;
}

export default async function analyticsRoutes(fastify: FastifyInstance, options: { supabase: SupabaseClient }) {
  const { supabase } = options;

  

  // Get overall API usage analytics for the authenticated user
  fastify.get('/api/usage/analytics', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      if (!usageData) return reply.send({ summary: {}, modelUsage: {}, dailyUsage: {}, recentActivity: [] });
  ...
  fastify.get('/api/usage/analytics/internal-chat', async (request: FastifyRequest, reply: FastifyReply) => {
  const userId = request.user.id;
  ...
  try {
    ...
    const recentUsage = internalChatUsage.filter(record => 
      new Date(record.created_at) >= twentyFourHoursAgo
    );
  ...
  fastify.get('/api/usage/analytics/:apiId', async (request: FastifyRequest<{ Params: ApiAnalyticsParams }>, reply: FastifyReply) => {
    const userId = request.user.id;
    ...
    try {
      ...
      if (usageError) throw usageError;
      if (!usageData) return reply.send({ apiInfo: {}, summary: {}, hourlyUsage: {}, recentActivity: [] });
